...
- Dodano do `.scroll-highlight` lazy-loaded animację GSAP ScrollTrigger + SplitText, która przy scrollowaniu wzmacnia kolejne słowa z `opacity: 0.4` do pełnego koloru. Zakres animacji został wydłużony od `top 75%` do `bottom 25%` i wygładzony przez `scrub: 1.2`. SplitText zachowuje pełne zdanie dla technologii asystujących, a przy ograniczeniu ruchu tekst wyświetla się od razu bez podziału i animacji. Stan początkowy `0.4` pozostaje świadomym ryzykiem kontrastu WCAG AA.
